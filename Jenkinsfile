pipeline {
    ...
    stages {
        ...
        stage('Create Grafana Dashboard') {
            ...
            steps {
                bat 'docker run -d -p 9092:3000 --name grafana grafana/grafana'
                bat 'sleep 10s'
                bat 'curl -X POST -H "Content-Type: application/json" -d "{\"name\":\"db\",\"type\":\"prometheus\",\"url\":\"http://192.168.85.1:9090\",\"access\":\"proxy\",\"isDefault\":true}" http://admin:pranay@1234@192.168.85.1:3000/api/datasources'
